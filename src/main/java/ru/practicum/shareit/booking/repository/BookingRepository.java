...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query(value = "select b from Booking b where b.item.ownerId = ?1")
    List<Booking> findAllByOwnerId(long ownerId, Sort sort);

    @Query(value = "select b from Booking b where b.item.ownerId = ?1 and b.status = ?2")
    List<Booking> findAllByOwnerIdAndStatus(long ownerId, Status status, Sort sort);

    @Query(value = "select b from Booking b where b.item.ownerId = ?1 and b.start > ?2")
    List<Booking> findAllByOwnerIdAndStartAfter(long ownerId, LocalDateTime localDateTime, Sort sort);

    @Query(value = "select b from Booking b where b.item.ownerId = ?1 and b.end < ?2")
    List<Booking> findAllByOwnerIdAndEndBefore(long ownerId, LocalDateTime localDateTime, Sort sort);

    @Query(value = "select b from Booking b where b.item.ownerId= ?1 and b.start < ?2 and b.end > ?2")
    ...
    Optional<Booking> findFirstByItemIdAndStatusAndStartAfterOrderByStart(Long id, Status status, LocalDateTime start);

    Optional<Booking> findFirstByItemIdAndStatusAndStartBeforeOrderByEndDesc(Long id, Status status, LocalDateTime start);

    Optional<Booking> findFirstByItemIdAndEndBeforeAndStatus(Long id, LocalDateTime start, Status status);
